...
export function ProviderCard({
  ...
  // Responsive layout handled with CSS media queries; no JS screen-size tracking.
  ...
  const renderStars = (rating: number | null | undefined) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(<Star key={i} className="h-3 w-3 sm:h-4 sm:w-4 lg:h-5 lg:w-5 fill-yellow-400 text-yellow-400" />)
    }

    if (hasHalfStar) {
      stars.push(<Star key="half" className="h-3 w-3 sm:h-4 sm:w-4 lg:h-5 lg:w-5 fill-yellow-400/50 text-yellow-400" />)
    }

    const remainingStars = 5 - Math.ceil(safeRating)
    for (let i = 0; i < remainingStars; i++) {
      stars.push(<Star key={`empty-${i}`} className="h-3 w-3 sm:h-4 sm:w-4 lg:h-5 lg:w-5 text-gray-300" />)
    }
  ...
  // Responsive List View Layout (mobile + desktop) with minimal duplication
  if (compact) {
    return (
      <div className="w-full max-w-[95vw] sm:max-w-[95vw] md:max-w-[95vw] lg:max-w-[95vw] mx-auto border border-gray-200 dark:border-gray-700 rounded-2xl mb-3 p-3 sm:p-4 lg:p-5 bg-white dark:bg-gray-900 hover:shadow-md transition-all">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 lg:gap-6 items-start">
          <div className="lg:col-span-4 space-y-2 order-1">
            <div className="flex items-start justify-between gap-2">
              <h3 className="font-bold text-base sm:text-xl text-foreground leading-tight break-words flex-1">
                {provider.name}
              </h3>
              {showDistance && provider.distance && (
                <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 px-2 sm:px-3 py-0.5 text-[11px] sm:text-sm whitespace-nowrap">
                  {formatDistance(provider.distance)}
                </Badge>
              )}
            </div>
            <Badge variant="outline" className="text-[11px] sm:text-sm px-2 sm:px-3 py-0.5 sm:py-1 w-fit">
              {provider.category}
            </Badge>
            ...
            <div className="flex items-start gap-2">
              ...
              <span className="text-xs sm:text-[13px] lg:text-sm text-foreground leading-relaxed">{provider.address}</span>
            </div>
          </div>

          <div className="lg:col-span-5 space-y-3 order-2">
            {services.length > 0 && (
              <div>
                <div className="grid grid-cols-3 gap-2 p-2 sm:p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                  <div className="text-center">
                    <div className="text-base sm:text-lg font-bold text-primary mb-0.5">
                      {services.length}
                    </div>
                    <div className="text-[10px] sm:text-[11px] text-muted-foreground uppercase tracking-wide">Total</div>
                  </div>
                  <div className="text-center">
                    <div className="text-base sm:text-lg font-bold text-green-600 mb-0.5">
                      {getFreeServices().length}
                    </div>
                    <div className="text-[10px] sm:text-[11px] text-muted-foreground uppercase tracking-wide">Free</div>
                  </div>
                  <div className="text-center">
                    <div className="text-base sm:text-lg font-bold text-orange-600 mb-0.5">
                      {getDiscountedServices().length}
                    </div>
                    <div className="text-[10px] sm:text-[11px] text-muted-foreground uppercase tracking-wide">Discounted</div>
                  </div>
                </div>
              </div>
            )}
            ...
            {topService && (
              <div className={`p-3 sm:p-3 rounded-xl border ${
                topService.is_free 
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-1.5">
                      {topService.is_discounted && !topService.is_free && (
                        <Badge className="bg-orange-600 text-white text-[10px] sm:text-xs px-1.5 py-0.5">
                          DISC
                        </Badge>
                      )}
                    </div>
                    <h4 className="font-bold text-sm sm:text-lg text-foreground mb-0.5 line-clamp-1">
                      {topService.name}
                    </h4>
                    {/* Removed category subtitle to save space */}
                    {topService.description && (
                      <p className="text-sm sm:text-sm text-foreground/80 mt-1 sm:mt-2 line-clamp-2">
                        {topService.description}
                      </p>
                    )}
                    {topService.price_info && topService.price_info.toLowerCase() !== 'not specified' && (
                      <div className="flex items-center gap-2 text-xs sm:text-sm mt-1 sm:mt-2">
                        ...
                        {topService.is_free ? (
                          <Badge className="bg-green-600 text-white px-1.5 py-0.5 text-[10px] sm:text-xs">FREE</Badge>
                        ) : (
          ...
          <div className="lg:col-span-3 space-y-2 order-3">
            <div>
              <h4 className="hidden sm:block text-xs font-semibold text-foreground uppercase tracking-wide mb-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {provider.phone && (
                <Button
                  ...
                  className="h-10 px-3 text-sm font-semibold bg-[#068282] hover:bg-[#0f766e] text-white rounded-xl whitespace-nowrap"
                >
              ...
              <Button
                ...
                className="h-10 px-3 text-sm font-semibold border-2 border-[#068282] text-[#068282] hover:bg-[#068282] hover:text-white rounded-xl whitespace-nowrap"
              >
            ...
            <div className="grid grid-cols-2 gap-2">
              <Button
                ...
                className="h-9 text-sm font-medium border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-xl whitespace-nowrap"
              >
              ...
              {provider.website ? (
                <a
                  ...
                  className="flex items-center justify-center h-9 text-sm font-medium text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-800 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl whitespace-nowrap"
                ...
                <Button
                  ...
                  variant="ghost"
                  disabled
                  className="h-9 text-sm font-medium border border-gray-200 dark:border-gray-700 rounded-xl opacity-50 whitespace-nowrap"
                >
                  <Globe className="h-4 w-4 mr-2" />
                  Website
                </Button>
  ...
  return (
    <Card className="w-full mx-auto hover:shadow-xl transition-all duration-300 border-2 hover:border-primary/30 group">
            ...
            <div className={`p-3 sm:p-4 lg:p-6 rounded-lg border-2 transition-all ${
              ...
              {topService.price_info && topService.price_info.toLowerCase() !== 'not specified' && (
                <div className="flex items-center gap-2 sm:gap-2.5 lg:gap-3 text-sm sm:text-base">
                  <span className="font-medium text-foreground">Pricing:</span>
                  {topService.is_free ? (
                    <Badge className="bg-green-600 text-white px-2 py-0.5">FREE</Badge>
                  ) : (
          ...
          <div className="flex gap-2 sm:gap-3 lg:gap-4">
            {provider.website && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex-1 flex items-center justify-center px-3 sm:px-4 lg:px-6 py-3 sm:py-3.5 lg:py-4 text-sm sm:text-base text-blue-600 hover:text-blue-800 hover:underline transition-colors rounded-md border border-transparent hover:bg-blue-50 min-h-[48px] sm:min-h-[52px] lg:min-h-[56px] whitespace-nowrap"
                onClick={() => onVisitWebsite?.(provider)}
              >
